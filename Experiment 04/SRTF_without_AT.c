...
int main() {
    ...
    while (completed < n) {
        ...
        // Find the process with the shortest remaining burst time among the non-completed processes
        for (int i = 0; i < n; i++) {
            if (!processes[i].completed && processes[i].remaining_time < shortest_remaining_time) {
